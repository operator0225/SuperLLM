...
## 어디서 무엇이 도는가 (중요: 학습 ≠ 배포)

증류 '학습'과 최종 모델 '배포'는 **다른 기계**에서 돈다. 섞으면 안 된다.

| | 학습(증류) | 배포(추론) |
|---|---|---|
| 기계 | NVIDIA GPU 워크스테이션/클라우드 | S25+ (Snapdragon) |
| 도는 모델 | 교사(Gemma E4B, 실제 ~8B) + 학생(QLoRA) | 최종 학생만 (~200MB q4_k_m) |
| 프레임워크 | PyTorch + bitsandbytes (**CUDA 필수**) | llama.cpp / LiteRT+QNN |
| 횟수 | 한 번(오프라인) | 상시 |

- **교사는 절대 폰에 올라가지 않는다.** 학습 시점에만 쓰는 자산이다.
- bitsandbytes NF4 / QLoRA 역전파는 **CUDA GPU 필요** → S25+에서 학습 불가.
- S25+는 배포 타깃. 최종 산출물은 증류된 학생을 q4_k_m(~200MB)로 양자화한 것뿐.
- 참고: Gemma 4 E4B 는 effective 4B 지만 실제 파라미터 ~8B. 배포엔 교사가 불필요.

### 교사를 q4_k_m로 돌릴 수 있나?
가능은 하다. llama.cpp `--logits-all` 로 전체 로짓을 낼 수 있어, **CUDA 없는 환경에서
교사 로짓을 오프라인 덤프**하는 데는 쓸 수 있다. 그러나 (a) 학생 QLoRA 역전파는 여전히
PyTorch+GPU 가 필요하고, (b) GPU 가 있으면 NF4 교사가 더 간단하다. 즉 q4_k_m 교사는
"CUDA 없는 오프라인 로짓 생성"에만 의미가 있으며 **폰 학습을 가능케 하지 않는다.**
